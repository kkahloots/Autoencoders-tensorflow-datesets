"""
AnnBetaVAE_graph.py:
Tensorflow Graph for the Annealed Beta-Variational Autoencoder
"""
...
'''
This is the Main Annealed BVAEGraph.
'''

class AnnBVAEGraph(BaseGraph):
    ...
    def train_epoch(self, session, x):
        tensors = [self.train_step, self.annbvae_loss, self.bvae_loss, self.annvae_loss, self.vae_loss, self.ae_loss, self.loss_reconstruction_m,
                   self.beta_reg, self.anneal_reg, self.div_cost_m,
                   self.L2_loss]
        feed_dict = {self.x_batch: x}
        _, annbvae_loss, bvae_loss, annvae_loss, vae_loss, ae_loss, loss_reconstruction_m, beta_reg, anneal_reg, div_cost_m, L2_loss = session.run(tensors, feed_dict=feed_dict)
        return annbvae_loss, bvae_loss, annvae_loss, vae_loss, ae_loss, loss_reconstruction_m, beta_reg, anneal_reg, div_cost_m, L2_loss

    def test_epoch(self, session, x):
        tensors = [self.annbvae_loss, self.bvae_loss, self.annvae_loss, self.vae_loss, self.ae_loss, self.loss_reconstruction_m,
                   self.beta_reg, self.anneal_reg, self.div_cost_m,
                   self.L2_loss]
        feed_dict = {self.x_batch: x}
        annbvae_loss, bvae_loss, annvae_loss, vae_loss, ae_loss, loss_reconstruction_m, beta_reg, anneal_reg, div_cost_m, L2_loss = session.run(tensors, feed_dict=feed_dict)
        return annbvae_loss, bvae_loss, annvae_loss, vae_loss, ae_loss, loss_reconstruction_m, beta_reg, anneal_reg, div_cost_m, L2_loss
