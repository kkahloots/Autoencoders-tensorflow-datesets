...
class VAEGraph(BaseGraph):
    ...
    def create_loss_optimizer(self):
        ...
        self.losses = ['ELBO_VAE', 'AE', 'Recons_{}'.format(self.config.reconst_loss), 'Div_{}'.format(self.config.div_cost),
                       'Regul_L2']
    ...
    def train_epoch(self, session, x):
        tensors = [self.train_step, self.vae_loss, self.ae_loss, self.loss_reconstruction_m, self.div_cost_m, self.L2_loss]
        feed_dict = {self.x_batch: x}
        _, loss, aeloss, recons, div, L2_loss = session.run(tensors, feed_dict=feed_dict)
        return loss, aeloss, recons, div, L2_loss
    
    def test_epoch(self, session, x):
        tensors = [self.vae_loss, self.ae_loss, self.loss_reconstruction_m, self.div_cost_m, self.L2_loss]
        feed_dict = {self.x_batch: x}
        loss, aeloss, recons, div, L2_loss  = session.run(tensors, feed_dict=feed_dict)
        return loss, aeloss, recons, div, L2_loss
