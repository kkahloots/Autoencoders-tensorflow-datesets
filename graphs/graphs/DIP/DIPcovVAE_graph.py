"""
DIPcovVAE_graph.py:
Disentangled Inferred Prior - covar
Tensorflow Graph for the DIP covar Variational Autoencoder
"""
...
'''
This is the Main DIPcovVAEGraph.
'''
class DIPcovVAEGraph(BaseGraph):
    ...
    def create_loss_optimizer(self):
        ...
        with tf.variable_scope('dipae_loss', reuse=self.config.reuse):
            self.covar_reg = tf.add(self.regularizer(self.encoder_mean, self.encoder_logvar), self.div_cost_m)
        ...
        self.losses = ['ELBO_DIPcovVAE', 'Regul_Covariance_Prior', 'VAE', 'AE', 'Recons_{}'.format(self.config.reconst_loss),
    ...
    def train_epoch(self, session, x):
        tensors = [self.train_step, self.dipvae_loss, self.covar_reg, self.vae_loss, self.ae_loss,
                   self.loss_reconstruction_m, self.div_cost_m, self.L2_loss]
        feed_dict = {self.x_batch: x}
        _, dipvae_loss, covar_reg, vae_loss, ae_loss, loss_recons, div, L2_loss  = session.run(tensors, feed_dict=feed_dict)
        return dipvae_loss, covar_reg, vae_loss, ae_loss, loss_recons, div, L2_loss
    
    def test_epoch(self, session, x):
        tensors = [self.dipvae_loss, self.covar_reg, self.vae_loss, self.ae_loss,
                   self.loss_reconstruction_m, self.div_cost_m, self.L2_loss]
        feed_dict = {self.x_batch: x}
        dipvae_loss, covar_reg, vae_loss, ae_loss, loss_recons, div, L2_loss  = session.run(tensors, feed_dict=feed_dict)
        return dipvae_loss, covar_reg, vae_loss, ae_loss, loss_recons, div, L2_loss
