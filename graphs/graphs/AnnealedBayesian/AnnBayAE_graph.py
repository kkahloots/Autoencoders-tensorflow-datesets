"""
AnnBayAE_graph.py:
Tensorflow Graph for the Annealed Bayesian Autoencoder
"""
...
'''
This is the Main Annealed BayAEGraph.
'''
class AnnBayAEGraph(BaseGraph):
    ...
    def create_loss_optimizer(self):
        ...
        with tf.variable_scope('annae_loss', reuse=self.config.reuse):
            ...
            self.anneal_reg = self.config.ann_gamma * tf.math.abs(self.bay_div - c)
        ...
        with tf.variable_scope("optimizer" ,reuse=self.config.reuse):
            ...
            self.train_step = self.optimizer.minimize(self.ann_bayae_loss, global_step=self.global_step_tensor)

        self.losses = ['ELBO_AnnBayAE',  'AnnAE', 'BayAE', 'AE', 'Recons_{}'.format(self.config.reconst_loss), 'Regul_L2', 'prior_recons_{}'.format(self.config.prior_reconst_loss),
    ...
    def train_epoch(self, session, x):
        tensors = [self.train_step, self.ann_bayae_loss, self.annae_loss, self.bayae_loss, self.ae_loss, self.loss_reconstruction_m, self.L2_loss,
                   self.prior_recons_m, self.bay_div]
        feed_dict = {self.x_batch: x}
        _, ann_bayae_loss, annae_loss, bayae_loss, aeloss, recons, L2_loss, prior_recons, bay_div = session.run(tensors, feed_dict=feed_dict)
        return ann_bayae_loss, annae_loss, bayae_loss, aeloss, recons, L2_loss, prior_recons, bay_div
    
    def test_epoch(self, session, x):
        tensors = [self.ann_bayae_loss, self.annae_loss, self.bayae_loss, self.ae_loss, self.loss_reconstruction_m, self.L2_loss,
                   self.prior_recons_m, self.bay_div]
        feed_dict = {self.x_batch: x}
        ann_bayae_loss, annae_loss, bayae_loss, aeloss, recons, L2_loss, prior_recons, bay_div  = session.run(tensors, feed_dict=feed_dict)
        return ann_bayae_loss, annae_loss, bayae_loss, aeloss, recons, L2_loss, prior_recons, bay_div
