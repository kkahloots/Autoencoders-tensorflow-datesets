"""
BayVAE_graph.py:
Tensorflow Graph for the Bayesian Variational Autoencoder
"""
...
'''
This is the Main Bayesian VAEGraph.
'''
class BayVAEGraph(BaseGraph):
    ...
    def create_loss_optimizer(self):
        ...
        with tf.variable_scope('bayae_loss', reuse=self.config.reuse):
            if self.config.isConv:
                self.bay_div = -1 * tf.reduce_mean(losses.get_divergence(self.encoder_mean, self.encoder_var, \
        ...
        with tf.variable_scope('optimizer', reuse=self.config.reuse):
            ...
            self.train_step = self.optimizer.minimize(self.bayvae_loss, global_step=self.global_step_tensor)

        self.losses = ['ELBO_BayVAE', 'BayAE', 'AE', 'Recons_{}'.format(self.config.reconst_loss), 'Div_{}'.format(self.config.div_cost),
                       'Regul_L2', 'prior_recons_{}'.format(self.config.prior_reconst_loss),
    ...
    def train_epoch(self, session, x):
        tensors = [self.train_step, self.bayvae_loss, self.bayae_loss, self.ae_loss, self.loss_reconstruction_m,
                   self.div_cost_m, self.L2_loss,
                   self.prior_recons_m, self.bay_div]
        feed_dict = {self.x_batch: x}
        _, bayvae_loss, bayae_loss, aeloss, recons, div_cost_m, L2_loss, prior_recons, bay_div = session.run(tensors, feed_dict=feed_dict)
        return bayvae_loss, bayae_loss, aeloss, recons, div_cost_m, L2_loss, prior_recons, bay_div

    def test_epoch(self, session, x):
        tensors = [self.bayvae_loss, self.bayae_loss, self.ae_loss, self.loss_reconstruction_m, self.div_cost_m, self.L2_loss,
                   self.prior_recons_m, self.bay_div]
        feed_dict = {self.x_batch: x}
        bayvae_loss, bayae_loss, aeloss, recons, div_cost_m, L2_loss, prior_recons, bay_div = session.run(tensors, feed_dict=feed_dict)
        return bayvae_loss, bayae_loss, aeloss, recons, div_cost_m, L2_loss, prior_recons, bay_div