...
class BaseGraph:
    ...
    def increment_cur_epoch_tensor(self):
        self.cur_epoch_tensor.assign(self.cur_epoch_tensor + 1)
    ...
    ------------------------------------------------------------------------------ 
    '''
    def create_net(self, input_, hidden_dim, output_dim, num_layers, transfer_fct, act_out, \
                   reuse, kinit, bias_init, drop_rate, prefix, isConv):
        net_function = ConvNet3 if isConv else DenseNet
        net_ = net_function(input_=input_,
